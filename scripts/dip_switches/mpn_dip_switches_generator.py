"""Tactile Switches Series Part Number Generator.

Generates part numbers and specifications for dip switches series.
...
import footprint_dip_switches_generator
import symbol_dip_switches_generator as symbol_dip_switches_generator
import symbol_dip_switches_specs as symbol_dip_switches_specs
...
def generate_files_for_series(
    series_name: str,
    unified_parts_list: list[symbol_dip_switches_specs.PartInfo],
    generated_footprints: set[str],
) -> None:
    """Generate CSV, KiCad symbol, and footprint files for a specific series.

    Generates all required files for a single dip switch series including
    ...
    Note:
        ...
        'dip_switches_footprints.pretty/' directories.

    """
    if series_name not in symbol_dip_switches_specs.SYMBOLS_SPECS:
        msg = f"Unknown series: {series_name}"
        raise ValueError(msg)

    specs = symbol_dip_switches_specs.SYMBOLS_SPECS[series_name]
    ...
    footprint_dir = "footprints/dip_switches_footprints.pretty"
    ...
    symbol_filename = f"DIP_SWITCHES_{series_code}_DATA_BASE.kicad_sym"

    # Generate part numbers and write to CSV
    parts_list = symbol_dip_switches_specs.PartInfo.generate_part_numbers(
        specs
    )
    ...
    try:
        symbol_dip_switches_generator.generate_kicad_symbol(
            csv_path,
    ...
    try:
        if parts_list:  # Check if parts_list is not empty
            ...
            if footprint_name not in generated_footprints:
                footprint_dip_switches_generator.generate_footprint_file(
...
def generate_unified_files(
    all_parts: list[symbol_dip_switches_specs.PartInfo],
    unified_csv: str,
    unified_symbol: str,
) -> None:
    ...
    database for all dip switch components.
...
if __name__ == "__main__":
    try:
        unified_parts: list[symbol_dip_switches_specs.PartInfo] = []
        generated_footprints: set[str] = set()  # Track generated footprints

        for series in symbol_dip_switches_specs.SYMBOLS_SPECS:
        ...
        UNIFIED_CSV = "UNITED_DIP_SWITCHES_DATA_BASE.csv"
        UNIFIED_SYMBOL = "UNITED_DIP_SWITCHES_DATA_BASE.kicad_sym"
