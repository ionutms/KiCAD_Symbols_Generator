"""Specifications for PCB seven segment display footprints.

This module defines the physical dimensions and parameters needed to generate
KiCad footprints for various seven segment displays.
It provides structured data types to represent display specifications
and a dictionary of pre-defined specifications for common display models.
...
class NonPlatedRoundMountingHoles(NamedTuple):
    """Defines the position and size of mounting holes for a display.

    Attributes:
        footprint_specs:
            List of mounting hole specifications,
            where each spec is [x, y, diameter]:
                - x: X position relative to the display origin
                - y: Y position relative to the display origin
...
class InternalCourtyard(NamedTuple):
    """Defines the internal courtyard dimensions for a display footprint.
...
class FootprintSpecs(NamedTuple):
    """Complete specifications for generating a display footprint.
    ...
    Attributes:
        ...
        pad_size:
            Size of pads.
            For through-hole: diameter/size of through-hole pads.
            For SMD: [width, height] for rectangular pads
        drill_size: Diameter of drill holes for through-hole components
        ...
        show_pin1_indicator: Whether to show pin 1 indicator
...
CONNECTOR_SPECS: dict[str, FootprintSpecs] = {
    ...
    "FYS-15011BUHR-21": FootprintSpecs(
        pad_pitch=2.54,
        body_dimensions=BodyDimensions(
            width_left=15.3,
            width_right=15.3,
