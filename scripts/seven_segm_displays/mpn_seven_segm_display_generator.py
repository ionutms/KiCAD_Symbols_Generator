"""Seven Segment Display Series Part Number Generator.

Generates part numbers and specifications for seven segment display series
...
import footprint_seven_segm_display_generator
import symbol_seven_segm_display_generator
...
def generate_files_for_series(
    series_name: str,
    unified_parts_list: list[symbol_seven_segm_displays_specs.PartInfo],
) -> None:
    ...
    Note:
        ...
        'seven_segm_display_footprints.pretty/' directories.

    """
    if series_name not in symbol_seven_segm_displays_specs.SYMBOLS_SPECS:
        msg = f"Unknown series: {series_name}"
        raise ValueError(msg)

    specs = symbol_seven_segm_displays_specs.SYMBOLS_SPECS[series_name]
    ...
    footprint_dir = "footprints/seven_segm_display_footprints.pretty"
    ...
    symbol_filename = f"SEVEN_SEGM_DISPLAYS_{series_code}_DATA_BASE.kicad_sym"

    parts_list = (
        symbol_seven_segm_displays_specs.PartInfo.generate_part_numbers(specs)
    )
    ...
    try:
        symbol_seven_segm_display_generator.generate_kicad_symbol(
            csv_path,
    ...
    try:
        for part in parts_list:
            footprint_seven_segm_display_generator.generate_footprint_file(
                part,
                footprint_dir,
            )
        ...
        print_message_utilities.print_error(
            f"Invalid display specification: {val_error}",
        )
    except OSError as io_error:
        print_message_utilities.print_error(
...
def generate_unified_files(
    all_parts: list[symbol_seven_segm_displays_specs.PartInfo],
    unified_csv: str,
    unified_symbol: str,
) -> None:
...
if __name__ == "__main__":
    try:
        unified_parts: list[symbol_seven_segm_displays_specs.PartInfo] = []

        for series in symbol_seven_segm_displays_specs.SYMBOLS_SPECS:
            print_message_utilities.print_info(
                f"\nGenerating files for {series} series:",
            )
            generate_files_for_series(series, unified_parts)

        UNIFIED_CSV = "UNITED_SEVEN_SEGM_DISPLAYS_DATA_BASE.csv"
        UNIFIED_SYMBOL = "UNITED_SEVEN_SEGM_DISPLAYS_DATA_BASE.kicad_sym"
