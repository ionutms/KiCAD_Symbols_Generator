"""Terminal Block Series Part Number Generator.

Generates part numbers and specifications for terminal block series
...
import footprint_terminal_block_generator
import symbol_terminal_block_generator
...
def generate_files_for_series(
    series_name: str,
    unified_parts_list: list[symbol_terminal_block_specs.PartInfo],
) -> None:
    ...
    Note:
        ...
        'terminal_block_footprints.pretty/' directories.

    """
    if series_name not in symbol_terminal_block_specs.SYMBOLS_SPECS:
        msg = f"Unknown series: {series_name}"
        raise ValueError(msg)

    specs = symbol_terminal_block_specs.SYMBOLS_SPECS[series_name]
    ...
    footprint_dir = "footprints/terminal_block_footprints.pretty"
    ...
    symbol_filename = f"TERMINAL_BLOCKS_{series_code}_DATA_BASE.kicad_sym"

    # Generate part numbers and write to CSV
    parts_list = symbol_terminal_block_specs.PartInfo.generate_part_numbers(
        specs
    )
    ...
    try:
        symbol_terminal_block_generator.generate_kicad_symbol(
            csv_path,
    ...
    try:
        for part in parts_list:
            footprint_terminal_block_generator.generate_footprint_file(
                part,
                footprint_dir,
            )
        ...
        print_message_utilities.print_error(
            f"Invalid terminal block specification: {val_error}",
        )
    except OSError as io_error:
        print_message_utilities.print_error(
...
def generate_unified_files(
    all_parts: list[symbol_terminal_block_specs.PartInfo],
    unified_csv: str,
    unified_symbol: str,
) -> None:
...
if __name__ == "__main__":
    try:
        unified_parts: list[symbol_terminal_block_specs.PartInfo] = []

        for series in symbol_terminal_block_specs.SYMBOLS_SPECS:
        ...
        UNIFIED_CSV = "UNITED_TERMINAL_BLOCKS_DATA_BASE.csv"
        UNIFIED_SYMBOL = "UNITED_TERMINAL_BLOCKS_DATA_BASE.kicad_sym"
