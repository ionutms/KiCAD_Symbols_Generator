"""Diodes Database Page.

This module provides a Dash page for viewing and interacting with diodes
specifications. It allows users to browse, search, and filter through a
database of diodes, with features for customizing the view and
accessing detailed information.

Key features:
- Interactive DataTable displaying diodes specifications
...
- Direct links to diodes datasheets
...
ag_grid_data: pd.DataFrame = pd.read_csv("data/UNITED_DIODES_DATA_BASE.csv")
total_rows = len(ag_grid_data)

TITLE = f"Diodes Database ({total_rows:,} items)"
ABOUT = (
    "The Diodes Database is an interactive web application that "
    "provides a comprehensive view of diode specifications.",
    "It allows users to easily browse, search, and filter "
    f"through a database of {total_rows:,} diodes, "
    "providing quick access to important information and datasheets.",
)

features = [
    "Interactive data table displaying diode specifications",
    "Dynamic filtering and multi-column sorting capabilities",
    "Customizable pagination with adjustable items per page",
    "Direct links to diode datasheets",
    "Responsive design adapting to light and dark themes",
    "Easy-to-use interface for exploring diode data",
    "Customizable column visibility",
]

usage_steps = [
    "Navigate to the Diodes Database page",
    "Use the table's built-in search functionality to find specific diodes",
    ...
    "Access diode datasheets by clicking on the provided links in the "
    "'Datasheet' column",
