"""Terminal Blocks Database Page.

This module provides a Dash page for viewing and interacting with terminal
block specifications. It allows users to browse, search, and filter through a
database of terminal blocks, with features for customizing the view and
accessing detailed information.

Key features:
- Interactive DataTable displaying terminal block specifications
...
- Direct links to terminal block datasheets
...
ag_grid_data: pd.DataFrame = pd.read_csv(
    "data/UNITED_TERMINAL_BLOCKS_DATA_BASE.csv"
)
total_rows = len(ag_grid_data)

TITLE = f"Terminal Blocks Database ({total_rows:,} items)"
ABOUT = (
    "The Terminal Blocks Database is an interactive web application that "
    "provides a comprehensive view of terminal block specifications.",
    "It allows users to easily browse, search, and filter "
    f"through a database of {total_rows:,} terminal blocks, "
    "providing quick access to important information and datasheets.",
)

features = [
    "Interactive data table displaying terminal block specifications",
    "Dynamic filtering and multi-column sorting capabilities",
    "Customizable pagination with adjustable items per page",
    "Direct links to terminal block datasheets",
    "Responsive design adapting to light and dark themes",
    "Easy-to-use interface for exploring terminal block data",
    "Customizable column visibility",
]

usage_steps = [
    "Navigate to the Terminal Blocks Database page",
    "Use the table's built-in search functionality "
    "to find specific terminal blocks",
    ...
    "Access terminal block datasheets by clicking on the provided links "
    "in the 'Datasheet' column",
