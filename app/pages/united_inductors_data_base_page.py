...
ag_grid_data: pd.DataFrame = pd.read_csv(
    "data/UNITED_INDUCTORS_DATA_BASE.csv"
)
...
def update_distribution_graph(
    ...
    figure_layout = {
        ...
        "title": {
            "text": "Indunctance Value Distribution "
    ...
    for tolerance in tolerances:
        ...
        figure.add_trace(
            go.Scatter(
                ...
                hovertemplate=(
                    "Inductance: %{x}<br>MPN: %{text}<extra></extra>"
                ),
            ),
        )
